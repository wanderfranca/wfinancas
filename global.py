...
if ("df_despesas.csv" in os.listdir()) and ("df_receitas.csv" in os.listdir()):
    ...
    df_fornecedores = pd.read_csv("df_fornecedores.csv", index_col=0, parse_dates=True)
...
if ("df_cat_receita.csv" in os.listdir()) and ("df_cat_despesa.csv" in os.listdir()):
    ...
    fornecedores = df_fornecedores.values.tolist()
    ...
    fornecedores = {'id':[999],'cpf_cnpj':[999],'Fornecedor': ['Geral']}
    df_fornecedores = pd.DataFrame(fornecedores)
    df_fornecedores.to_csv("df_fornecedores.csv")
